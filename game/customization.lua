...
local currentCamera
local cameraHandle
client.currentCamPoint = nil
local dofActive = false

local function applyDof(cam)
    local dof = Config.DepthOfField
    if not dof or not dof.enabled then return end

    SetCamUseShallowDofMode(cam, true)
    SetCamNearDof(cam, dof.nearDof)
    SetCamFarDof(cam, dof.farDof)
    SetCamDofStrength(cam, dof.strength)
    SetCamDofFnumberOfLens(cam, dof.fnumberOfLens)
    SetCamDofFocusDistanceBias(cam, dof.focusDistanceBias)
    SetCamDofMaxNearInFocusDistanceBlendLevel(cam, 1.0)
end
...
local function setCamera(key)
    ...
    client.currentCamPoint = camPoint
...
function client.isCameraInterpolating()
    return isCameraInterpolating
end

function client.getCameraHandle()
    return cameraHandle
end
...
local function playCustomizationAnim()
    TaskStandStill(cache.ped, -1)
end

local function stopCustomizationAnim()
