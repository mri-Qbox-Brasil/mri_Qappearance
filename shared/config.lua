...
-- mri_Qadmin, mri_Qloadscreen, mri_Qchat, ox_lib, ox_inventory). Definida via
-- `setr mri:color "#hex"` no server.cfg ou pelo painel admin do mri_Qadmin.
-- Server-side broadcasta mudancas em runtime via TriggerClientEvent.
...
-- Depth of Field (DOF) - Efeito de desfoque no fundo durante a customização
Config.DepthOfField = {
    enabled = true,            -- Ativar/desativar o efeito de DOF
    nearDof = 0.5,             -- Distância mínima de foco (metros)
    farDof = 5.0,              -- Distância máxima de foco (metros)
    strength = 1.0,            -- Intensidade do desfoque (0.0 a 1.0)
    fnumberOfLens = 1.2,       -- Abertura da lente (menor = mais desfoque no fundo)
    focusDistanceBias = 0.0,   -- Viés da distância focal
}
